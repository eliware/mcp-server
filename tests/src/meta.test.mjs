...
test('uses package fallback values', () => expect(loadPackageMeta({ log: { warn: jest.fn() }, readFile: () => JSON.stringify({}), packagePath: 'test' })).toEqual({ name: '@eliware/mcp-server', version: '1.0.0' }));
test('logs and falls back when metadata cannot be read', () => { const warn=jest.fn(); expect(loadPackageMeta({ log:{warn}, readFile:()=>{throw new Error('bad')}})).toEqual({name:'@eliware/mcp-server',version:'1.0.0'}); expect(warn).toHaveBeenCalled(); });
